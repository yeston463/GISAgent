...
def _proposed_green_space():
    return {
        ...
        "features": [{
            ...
            "geometry": {
                "type": "Polygon",
                "coordinates": [[[121.47355,31.23210],[121.47555,31.23210],[121.47555,31.23275],[121.47355,31.23275],[121.47355,31.23210]]],
            },
        }],
    }


def evaluate_demo_case():
    case_data = _load_demo_json("case.json")
    rule_set = _load_demo_json("rules.json")
    aoi = case_data["aoi"]
    current_buildings = case_data["buildings"]
    current_green = case_data["greenSpaces"]
    current_metrics = calculate_metrics({"aoi": aoi, "buildings": current_buildings})
    if current_metrics.get("status") != "Success":
        return current_metrics

    site_area = _safe_float(current_metrics.get("site_area"), 0.0) or 0.0
    current_green_area = _polygon_area_sqm(current_green, aoi)
    current_green_rate = current_green_area / site_area * 100.0 if site_area > 0 else 0.0
    current_evaluations, current_passed = _evaluate_demo_metrics(current_metrics, current_green_rate, rule_set)
    problems, _ = _problem_buildings(current_buildings, rule_set["rules"])

    optimized_buildings, building_changes = _optimized_buildings(current_buildings, rule_set["rules"])
    proposed_green = _proposed_green_space()
    optimized_green = _feature_collection(
        _features_from_source(current_green) + _features_from_source(proposed_green)
    )
    optimized_metrics = calculate_metrics({"aoi": aoi, "buildings": optimized_buildings})
    optimized_green_area = _polygon_area_sqm(optimized_green, aoi)
    optimized_green_rate = optimized_green_area / site_area * 100.0 if site_area > 0 else 0.0
    optimized_evaluations, optimized_passed = _evaluate_demo_metrics(optimized_metrics, optimized_green_rate, rule_set)

    current_metrics["green_area"] = round(current_green_area, 2)
    current_metrics["green_rate"] = round(current_green_rate, 2)
    optimized_metrics["green_area"] = round(optimized_green_area, 2)
    optimized_metrics["green_rate"] = round(optimized_green_rate, 2)

    return {
        "status": "Success",
        "stage": "planning_evaluation",
        "far": current_metrics.get("far"),
        "building_count": current_metrics.get("building_count"),
        "site_area": current_metrics.get("site_area"),
        "building_area": current_metrics.get("building_area"),
        "building_density": current_metrics.get("building_density"),
        "green_rate": round(current_green_rate, 2),
        ...
        "optimized": {"metrics": optimized_metrics, "evaluations": optimized_evaluations, "passed": optimized_passed},
        "comparison": {
            "far": {"before": current_metrics.get("far"), "after": optimized_metrics.get("far")},
            "buildingDensity": {"before": current_metrics.get("building_density"), "after": optimized_metrics.get("building_density")},
            "buildingHeight": {"before": (current_metrics.get("height_stats") or {}).get("max"), "after": (optimized_metrics.get("height_stats") or {}).get("max")},
            "greenRate": {"before": round(current_green_rate, 2), "after": round(optimized_green_rate, 2)},
        },
        "changes": building_changes + [{"action": "add_green_space", "featureId": "G02", "name": "建议新增公共绿地"}],
        "aoi": aoi,
        "buildings": current_buildings,
        "greenSpaces": current_green,
        "problemBuildings": problems,
        "optimizedBuildings": optimized_buildings,
        "optimizedGreenSpaces": optimized_green,
        "proposedGreenSpace": proposed_green,
        "commands": [
            {"action": "addGeoJsonLayer", "params": {"layerId": "demo-aoi", "title": "比赛案例红线", "data": aoi, "style": "aoi", "visible": True}},
            {"action": "addGeoJsonLayer", "params": {"layerId": "existing-scenario", "title": "现状建筑方案", "data": current_buildings, "style": "existing", "visible": True}},
            {"action": "addGeoJsonLayer", "params": {"layerId": "existing-green", "title": "现状绿地", "data": current_green, "style": "existingGreen", "visible": True}},
            {"action": "addGeoJsonLayer", "params": {"layerId": "problem-buildings", "title": "问题建筑", "data": problems, "style": "warning", "visible": False}},
            {"action": "addGeoJsonLayer", "params": {"layerId": "optimized-scenario", "title": "优化建筑方案", "data": optimized_buildings, "style": "optimized", "visible": False}},
            {"action": "addGeoJsonLayer", "params": {"layerId": "optimized-green", "title": "优化绿地方案", "data": optimized_green, "style": "green", "visible": False}},
            {"action": "switchPlanningScenario", "params": {"scenario": "existing"}},
            {"action": "comparePlanningScenarios", "params": {"comparison": {
                "far": {"before": current_metrics.get("far"), "after": optimized_metrics.get("far")},
                "buildingDensity": {"before": current_metrics.get("building_density"), "after": optimized_metrics.get("building_density")},
                "buildingHeight": {"before": (current_metrics.get("height_stats") or {}).get("max"), "after": (optimized_metrics.get("height_stats") or {}).get("max")},
                "greenRate": {"before": round(current_green_rate, 2), "after": round(optimized_green_rate, 2)}
        ...
        "message": "规划评价与优化方案复算完成。演示规则不是法定审批依据。",
    }


@app.post("/analysis/demo_case/evaluate")
async def evaluate_planning_demo():
    try:
        return evaluate_demo_case()
    except Exception as exc:
